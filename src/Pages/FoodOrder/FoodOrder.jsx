...
const FoodOrder = () => {
  ...
  const { currentFood: food, loading: foodLoading } = useSelector(
    (state) => state.food,
  );
  const { loading: orderLoading, error } = useSelector(
    (state) => state.foodActions,
  );
  ...
  const handleQuantityChange = (e) => {
    ...
    if (newQuantity >= 1 && newQuantity <= parseInt(display.foodQuantity)) {
      ...
      showToast(
        `Maximum available quantity is ${display.foodQuantity}`,
        "error",
      );
    } else {
      setQuantity(1);
  ...
  const handlePurchase = async (orderData) => {
    try {
      ...
      if (quantity <= 0 || quantity > display.foodQuantity) {
        showToast(
          `Please select a quantity between 1 and ${display.foodQuantity}`,
          "error",
        );
        return;
      }
      ...
      showToast(
        error.message || "Error placing order. Please try again.",
        "error",
      );
    }
  };
  ...
  return (
    <>
      ...
      {!display && (
        <div className="text-center py-10">Loading food details...</div>
      )}
                ...
                <div className="space-y-4">
                  ...
                  <div className="w-full">
                    ...
                    <div className="flex items-center">
                      <span className="bg-gray-200 rounded-l-lg p-2.5 text-gray-600">
                        $
                      </span>
                      <input
                        type="text"
