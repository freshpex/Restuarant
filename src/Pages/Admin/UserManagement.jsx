...
import { FaUser, FaUserShield, FaUserCog, FaSpinner, FaEllipsisV, FaUserEdit, FaTrashAlt, FaUtensils, FaMoneyBillWave } from 'react-icons/fa';
...
const UserManagement = () => {
  ...
  const saveMobileRoleChange = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/admin/users/${mobileEditingUser.email}/role`, {
        method: 'PATCH',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
      ...
      toast.success(`Updated ${mobileEditingUser.name || mobileEditingUser.email}'s role to ${newRole}`);
      
      // Update local state
      setUsers(prevUsers => 
        prevUsers.map(user => 
          user.email === mobileEditingUser.email ? { ...user, role: newRole } : user
        )
      );
    } catch (error) {
      toast.error(error.message || 'Error updating user role');
                      ...
                      <tr key={`desktop-${user._id || user.email}`}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                          ...
                          <button
                            ...
                            disabled={deletingUser === user.email || user.email === localStorage.getItem('userEmail')}
                          >
                            <FaTrashAlt />
                          </button>
                        </td>
                          ...
                          <div className="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg z-10 border border-gray-200">
                            {updating[user.email] ? (
                              ...
                              <>
                                ...
                                >
                                  <FaUserEdit className="inline mr-2" /> Edit Role
                                </button>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-lg p-6 max-w-sm w-full mx-4">
            ...
            <p className="mb-4 text-sm text-gray-600">
              Update role for <span className="font-semibold">{mobileEditingUser.name || mobileEditingUser.email}</span>
            </p>
