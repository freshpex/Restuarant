...
const OrderManagement = () => {
  ...
  const groupOrdersByReference = (ordersList) => {
    ...
    ordersList.forEach((order) => {
      const groupIdentifier =
        order.orderGroupId || order.orderReference || order._id;
      ...
      groupedOrdersMap[groupIdentifier].totalAmount += parseFloat(
        order.totalPrice || 0,
      );
    ...
    try {
      ...
      if (orderGroup && orderGroup.items && orderGroup.items.length > 0) {
        const updatePromises = orderGroup.items.map(async (item) => {
          const response = await fetch(
            `${API_URL}/admin/orders/${item._id}/status`,
            {
              method: "PATCH",
              headers: {
                Authorization: `Bearer ${token}`,
  ...
  const updatePaymentStatus = async (orderId, paymentStatus) => {
    try {
      ...
      if (orderGroup && orderGroup.items && orderGroup.items.length > 0) {
        const updatePromises = orderGroup.items.map(async (item) => {
          const response = await fetch(
            `${API_URL}/admin/orders/${item._id}/payment-status`,
            {
              method: "PATCH",
              headers: {
                Authorization: `Bearer ${token}`,
        ...
        await Promise.all(updatePromises);
        toast.success(
          `Payment status for all items updated to ${paymentStatus}`,
        );
  ...
  const filteredOrders = groupedOrders.filter((order) => {
    ...
    if (dateRange.start || dateRange.end) {
      ...
      if (dateRange.start && dateRange.end) {
        matchesDate = orderDate.isBetween(
          dateRange.start,
          dateRange.end,
          "day",
          "[]",
  ...
  const deleteOrder = async (
    ...
    try {
      ...
      if (isBulkOrder) {
        ...
        toast.success(
          responseData.message || "Bulk order deleted successfully",
        );
      } else {
          ...
          const updatedGroups = prevGroups.map((group) => {
            if (group.items.some((item) => item._id === actualId)) {
              return {
                ...
                isBulkOrder:
                  group.items.filter((item) => item._id !== actualId).length >
                  1,
              };
            }
            return group;
  ...
  const deleteAllOrders = async () => {
    try {
      ...
      setOrders([]);
      toast.success(
        `${data.deletedCount || "All"} orders deleted successfully`,
      );
    } catch (error) {
                  ...
                  >
                    ...
                    >
                      ...
                      <div className="flex items-center space-x-2">
                        <span
                          className={`px-2 py-1 inline-flex text-xs font-semibold rounded-full ${statusColors[order.status]}`}
                        >
                          {capitalizeWords(order.status)}
                        </span>
                        <FaChevronDown
                          className={`text-gray-400 transition-transform ${expandedOrder === order._id ? "transform rotate-180" : ""}`}
                        />
                      </div>
                    </div>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {currentItems.length === 0 ? (
                    ...
                    currentItems.map((order) => (
                      <tr
                        key={order._id}
                        className={order.isBulkOrder ? "bg-purple-50" : ""}
                      >
                            ...
                            <div className="flex items-center">
                              {order.items && order.items.length > 0 ? (
                                <div className="flex items-center">
                                  ...
                                  <div className="ml-2">
                                    <div className="text-sm text-gray-900">
                                      {getItemName(order.items[0])}
                                    </div>
                                    <div className="text-xs text-gray-500">
                                      Qty: {order.items[0].quantity}
                                    </div>
                                    <div className="text-xs font-medium">
                                      {formatPrice(order.items[0].totalPrice)}
                                    </div>
                                  </div>
                                </div>
                              ) : (
          ...
          <div className="bg-white rounded-lg shadow-xl w-full max-w-md">
            ...
            <div className="p-5">
              ...
              <p className="mb-4 text-gray-700">
                Are you absolutely sure you want to delete all {orders.length}{" "}
