...
const StaffActivities = () => {
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute(
      "download",
      `staff-activities-${startDate}-to-${endDate}.csv`,
    );
  ...
  const deleteActivity = async (id) => {
    ...
    try {
      ...
      const response = await axios.delete(
        `${API_URL}/admin/staff-activities/${id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const deleteAllActivities = async () => {
    if (
      !confirm(
        "Are you sure you want to delete ALL staff activities? This action cannot be undone.",
      )
    ) {
      return;
    }
    ...
    if (
      !confirm(
        "FINAL WARNING: This will permanently delete ALL activity records. Continue?",
      )
    ) {
      return;
    }
    ...
    try {
      ...
      if (response.data.success) {
        toast.success(
          `${response.data.deletedCount || "All"} activities deleted successfully`,
        );
        fetchStaffActivities();
      } else {
              ...
              <>
                <div className="space-y-6">
                  {paginatedActivities.map((activity, index) => (
                    <div
                      key={index}
                      className={`flex gap-3 border-b border-gray-100 pb-6 last:border-0 ${activity.isDeletionRecord ? "bg-red-50 p-3 rounded" : ""}`}
                    >
                      <div className="mt-1">
                          ...
                          activity.details?.foodName && (
                            <div className="mt-2 bg-gray-50 p-2 rounded text-sm">
                              ...
                              <div className="flex justify-between mt-1">
                                <span>
                                  Price: ₦{activity.details?.foodPrice}
                                </span>
                                <span>
                                  Quantity: {activity.details?.foodQuantity}
                                </span>
                              </div>
                            </div>
                          )}
                          ...
                          activity.details?.orderDetails?.customer && (
                            <div className="mt-2 bg-gray-50 p-2 rounded text-sm">
                              <div className="flex justify-between">
                                <span>
                                  Order:{" "}
                                  {activity.details?.orderId?.substring(0, 8)}
                                </span>
                                <span>
                                  Customer:{" "}
                                  {activity.details?.orderDetails?.customer}
                              ...
                              {activity.activityType ===
                                ...
                                <p className="mt-1">
                                  Payment status:{" "}
                                  <span className="font-medium">
                                    {activity.details?.newPaymentStatus}
                                  </span>
                                </p>
                              )}
                            </div>
