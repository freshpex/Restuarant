...
import { 
  ...
  FaPlus, FaTrash, FaTimes, FaGlassWhiskey,
  FaHamburger, FaToggleOn, FaToggleOff
} from 'react-icons/fa';
...
const StaffAddOrder = () => {
  ...
  const handleQuantityChange = (itemId, newQuantity) => {
    const updatedItems = selectedItems.map(item => {
      if (item.id === itemId) {
        ...
        const price = item.type === 'food' ? parseFloat(item.foodPrice) : parseFloat(item.drinkPrice);
  ...
  const createOrder = async (e) => {
    ...
    try {
      ...
      if (selectedItems.length === 1) {
        ...
        toast.success(`Order with ${selectedItems.length} items created successfully!`);
      }
  ...
  const drinkCount = selectedItems.filter(item => item.type === 'drink').length;
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              {/* Order Details */}
                      ...
                      <div className="flex justify-between items-center mb-4">
                        ...
                        <span className="text-xs text-gray-500 mb-1">Click the button to choose between foods/drinks</span>
                        <button
                          type="button"
                      ...
                      <div className="mb-6 relative" ref={searchRef}>
                        ...
                        {itemType === 'food' ? (
                          <>
                          <FaHamburger className="inline-block mr-1" /> Add Food Items
                          </>
                        ) : (
                          <>
                          <FaGlassWhiskey className="inline-block mr-1" /> Add Drink Items
                          </>
                        )}
                    ...
                    {showSearchResults && searchResults.length > 0 && (
                      <div className="absolute z-10 w-full mt-1 bg-white border border-gray-300 shadow-lg rounded-md max-h-60 overflow-y-auto">
                        {searchResults.map(item => {
                          const name = item.type === 'food' ? item.foodName : item.drinkName;
                          const image = item.type === 'food' ? item.foodImage : item.drinkImage;
                          const price = item.type === 'food' ? item.foodPrice : item.drinkPrice;
                          const quantity = item.type === 'food' ? item.foodQuantity : item.drinkQuantity;
                            ...
                            >
                              ...
                              <div className="ml-3">
                                ...
                                <div className="flex text-xs text-gray-500">
                                  <span className="mr-4">{formatPrice(price)}</span>
                    ...
                    {showSearchResults && searchQuery && searchResults.length === 0 && (
                      <div className="absolute z-10 w-full mt-1 bg-white border border-gray-300 shadow-lg rounded-md p-3">
                        <p className="text-gray-500">No matching {itemType} items found</p>
                      </div>
                    )}
                  </div>
                  ...
                  {isLoading ? (
                    ...
                    <div className="text-sm text-gray-500 mt-2">
                      {itemType === 'food' ? (
                        <span>Tip: Search by name or category from {availableFoods.length} available food items</span>
                      ) : (
                        <span>Tip: Search by name or category from {availableDrinks.length} available drink items</span>
                      )}
                    </div>
                  )}
                </div>
                ...
                {selectedItems.length > 0 && (
                  <div className="mb-6 border rounded-md divide-y">
                    {selectedItems.map((item, index) => {
                      ...
                      const image = item.type === 'food' ? item.foodImage : item.drinkImage;
                      const price = item.type === 'food' ? item.foodPrice : item.drinkPrice;
                      const category = item.type === 'food' ? item.foodCategory : item.drinkCategory;
                ...
                <div className="mb-4">
                  <p className="text-sm text-gray-500 mb-2">
                    {selectedItems.length === 0 ? 'Search and add items to the order' : 'You can add more items to this order'}
                  </p>
                </div>
                ...
                {selectedItems.length > 0 && (
                  <div className="bg-yellow-50 p-4 rounded-md mb-6 border border-yellow-100">
                    <h4 className="font-medium text-yellow-800 mb-2">Order Summary ({selectedItems.length} {selectedItems.length === 1 ? 'item' : 'items'})</h4>
                    <div className="space-y-2 max-h-40 overflow-y-auto mb-2">
                      {selectedItems.map(item => (
                        <div key={item.id} className="flex justify-between text-sm">
                          <span className="text-gray-700">
                            {item.type === 'food' ? item.foodName : item.drinkName} x {item.quantity}
                          </span>
                ...
                <div className="flex items-center justify-end space-x-3 pt-4 border-t">
                  ...
                  <button
                    type="submit"
                    disabled={isCreatingOrder || selectedItems.length === 0 || Object.keys(quantityErrors).some(key => quantityErrors[key])}
                    className={`px-6 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-yellow-600 hover:bg-yellow-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-yellow-500 ${
                      (isCreatingOrder || selectedItems.length === 0 || Object.keys(quantityErrors).some(key => quantityErrors[key])) ? 'opacity-70 cursor-not-allowed' : ''
                    }`}
                  >
                    {isCreatingOrder ? (
                      <>
