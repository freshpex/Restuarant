      ...
      if (orderGroup && orderGroup.items && orderGroup.items.length > 0) {
        const updatePromises = orderGroup.items.map(async (item) => {
          const response = await fetch(
            `${API_URL}/staff/orders/${item._id}/status`,
            {
              method: "PATCH",
              headers: {
                Authorization: `Bearer ${token}`,
        ...
        const response = await fetch(
          `${API_URL}/staff/orders/${orderId}/status`,
          {
            method: "PATCH",
            headers: {
              Authorization: `Bearer ${token}`,
  ...
  const updatePaymentStatus = async (orderId, paymentStatus) => {
    try {
      ...
      if (orderGroup && orderGroup.items && orderGroup.items.length > 0) {
        const updatePromises = orderGroup.items.map(async (item) => {
          const response = await fetch(
            `${API_URL}/staff/orders/${item._id}/payment-status`,
            {
              method: "PATCH",
              headers: {
                Authorization: `Bearer ${token}`,
        ...
        const response = await fetch(
          `${API_URL}/staff/orders/${orderId}/payment-status`,
          {
            method: "PATCH",
            headers: {
              Authorization: `Bearer ${token}`,
                          ...
                          <div className="mb-3">
                            ...
                            {order.paymentStatus === "paid" && (
                              <p className="text-xs text-gray-500 mt-1">
                                Payment already completed and cannot be changed,
                                For any issue tell the Admin
                              </p>
                            )}
