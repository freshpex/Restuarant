...
export default function TopDrinkCard({ drink: drink }) {
  const dispatch = useDispatch();
  const cartItems = useSelector(state => state.cart.items);
  
  const itemInCart = cartItems.find(item => item._id === drink._id);
  const currentCartQty = itemInCart ? itemInCart.quantity : 0;
  const availableQty = parseInt(drink.drinkQuantity) || 0;

  const handleAddToCart = () => {
    if (availableQty <= 0) {
      toast.error(`${drink.drinkName} is currently out of stock!`);
      return;
    }
    
    if (currentCartQty >= availableQty) {
      toast.error(`Cannot add more ${drink.drinkName}. Only ${availableQty} available in stock!`);
      return;
    }
    ...
    toast.success(`${drink.drinkName} added to cart!`);
  };
    ...
    <Card className="w-full max-w-[24rem] shadow-lg mx-auto">
      ...
      <CardBody>
        <div className="mb-3 flex items-center justify-between">
          ...
          <Typography variant="h5" color="blue-gray" className="font-medium">
          {formatPrice(parseFloat(drink.drinkPrice))}
          </Typography>
        </div>
        <Typography color="gray" className="font-normal mb-2 line-clamp-2">
          {drink.drinkDescription || "A delicious meal prepared with the finest ingredients."}
        </Typography>
        <div className="flex items-center justify-between">
          <Typography variant="small" color="blue-gray">
            {drink.drinkCategory || "Main Course"}
          </Typography>
      ...
      <CardFooter className="pt-3 flex gap-2">
        <Link to={`/topDrink/${drink._id}`} className="flex-1">
