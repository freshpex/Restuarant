        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="bg-white rounded-lg shadow-md p-6 mb-8">
            ...
            <div className="mt-3 text-xs text-gray-500">
              <p>
                Enter the order reference from your receipt or confirmation
                email (starting with "TK-")
              </p>
            </div>
          </div>
            ...
            <div className="bg-white rounded-lg shadow-md p-6 mb-8">
              <div className="flex flex-col items-center justify-center py-8">
                ...
                <p className="text-gray-600 text-center max-w-md">
                  {error} Please double-check your order reference number and
                  try again.
                </p>
              </div>
            </div>
          ...
          {order && !loading && (
            <div className="bg-white rounded-lg shadow-md overflow-hidden">
              ...
              {(() => {
                const { food: foodCount, drink: drinkCount } =
                  getItemTypeCounts(order.items);
              ...
              <div className="px-6 pt-6">
                ...
                <div className="relative">
                  ...
                  <div className="flex justify-between text-xs text-gray-600">
                    <div
                      className={`flex flex-col items-center ${getStatusStep(order.status) >= 1 ? "text-green-600" : ""}`}
                    >
                      <div
                        className={`rounded-full p-1 ${getStatusStep(order.status) >= 1 ? "bg-green-100" : "bg-gray-100"} mb-1`}
                      >
                        <FaShoppingBag
                          className={`${getStatusStep(order.status) >= 1 ? "text-green-600" : "text-gray-400"}`}
                        />
                      </div>
                      <span>Confirmed</span>
                    </div>
                    <div
                      className={`flex flex-col items-center ${getStatusStep(order.status) >= 2 ? "text-green-600" : ""}`}
                    >
                      <div
                        className={`rounded-full p-1 ${getStatusStep(order.status) >= 2 ? "bg-green-100" : "bg-gray-100"} mb-1`}
                      >
                        <FaUtensils
                          className={`${getStatusStep(order.status) >= 2 ? "text-green-600" : "text-gray-400"}`}
                        />
                      </div>
                      <span>Preparing</span>
                    </div>
                    <div
                      className={`flex flex-col items-center ${getStatusStep(order.status) >= 3 ? "text-green-600" : ""}`}
                    >
                      <div
                        className={`rounded-full p-1 ${getStatusStep(order.status) >= 3 ? "bg-green-100" : "bg-gray-100"} mb-1`}
                      >
                        <FaBox
                          className={`${getStatusStep(order.status) >= 3 ? "text-green-600" : "text-gray-400"}`}
                        />
                      </div>
                      <span>Ready</span>
                    </div>
                    <div
                      className={`flex flex-col items-center ${getStatusStep(order.status) >= 4 ? "text-green-600" : ""}`}
                    >
                      <div
                        className={`rounded-full p-1 ${getStatusStep(order.status) >= 4 ? "bg-green-100" : "bg-gray-100"} mb-1`}
                      >
                        <FaMotorcycle
                          className={`${getStatusStep(order.status) >= 4 ? "text-green-600" : "text-gray-400"}`}
                        />
                      </div>
                      <span>Delivered</span>
                    </div>
                  ...
                  <div>
                    ...
                    <div className="bg-white p-4 rounded-lg border border-gray-200">
                      ...
                      <div className="mb-2">
                        ...
                        <p
                          className={`${order.paymentStatus === "paid" ? "text-green-600" : "text-yellow-600"} font-medium`}
                        >
                  ...
                  <div className="flex">
                    <div className="ml-3">
                      ...
                      <div className="mt-1 text-sm text-blue-700">
                        <p>
                          Create an account to easily track all your orders and
                          save your delivery information for future orders.
                        </p>
                        <div className="mt-3">
                          <Link
                            to="/signup"
          ...
          {!order && !error && !loading && (
            <div className="bg-yellow-50 border border-yellow-100 rounded-lg p-6 text-center">
              ...
              <p className="text-gray-600">
                Enter your order ID, reference number or transaction ID to track
                your order
              </p>
            </div>
          )}
