...
const Cart = () => {
  ...
  useEffect(() => {
    ...
    addressChangeTimeout.current = setTimeout(() => {
      ...
      const finalLocation =
        isNightDelivery &&
        ["emaudo", "town", "village", "irrua"].includes(detectedLocation)
          ? "night" +
            detectedLocation.charAt(0).toUpperCase() +
            detectedLocation.slice(1)
          : detectedLocation;

      const newLocation =
        finalLocation || (isNightDelivery ? "nightVillage" : "village");

      if (deliveryLocation !== newLocation && newLocation) {
        const oldLocationName =
          deliveryLocation.replace("night", "").charAt(0).toUpperCase() +
          deliveryLocation.replace("night", "").slice(1);

        const newLocationName =
          newLocation.replace("night", "").charAt(0).toUpperCase() +
          newLocation.replace("night", "").slice(1);
  ...
  useEffect(() => {
    ...
    if (isNight) {
      ...
      if (nightEquivalents[deliveryLocation]) {
        ...
        showToast(
          "Night delivery hours (7pm-6am) detected. Additional ₦200 fee applies.",
          {
            duration: 5000,
            icon: "🌙",
          },
  ...
  const validateLocationWithAddress = (location, address) => {
    ...
    const locationChecks = {
      emaudo:
        !addressLower.includes("emaudo") && !addressLower.includes("campus"),
      town: !["market square", "alli square", "dublous", "town", "market"].some(
        (k) => addressLower.includes(k),
      ),
      irrua:
        !addressLower.includes("irrua") &&
        !addressLower.includes("hospital") &&
        !addressLower.includes("isth"),
    ...
    if (locationChecks[locationWithoutNight]) {
      const locationName =
        locationWithoutNight.charAt(0).toUpperCase() +
        locationWithoutNight.slice(1);
      showToast(
      ...
      <>
        ...
        <div className="min-h-screen bg-gray-50 pt-24 pb-12 px-4 sm:px-6 lg:px-8">
          <div className="max-w-4xl mx-auto">
            <h1 className="text-3xl font-bold text-gray-900 mt-64 text-center">
              Your Cart
            </h1>
            ...
            <div className="lg:col-span-2 bg-white rounded-lg shadow-md p-6">
              <div className="divide-y divide-gray-200">
                {cartItems.map((item) => (
                  <div
                    key={item._id}
                    className="flex py-6 flex-col sm:flex-row"
                  >
              ...
              <div className="space-y-4">
                ...
                <div className="mt-4">
                  ...
                  <p className="mt-1 text-xs text-gray-500">
                    ...
                    Include location details (Emaudo, Town, Irrua, etc.)
                  </p>
                </div>
                ...
                <div className="mt-4">
                  ...
                  {isNightDelivery && (
                    <p className="mt-1 text-xs text-yellow-600 bg-yellow-50 p-2 rounded border border-yellow-100">
                      <strong>Night Delivery:</strong> An additional ₦200 fee
                      applies for deliveries between 7pm and 6am for your safety
                      and convenience.
                    </p>
                  )}

                  <p className="mt-1 text-xs text-gray-500">
                    Delivery fee will be added to your order total. We'll try to
                    auto-detect your location from your address.
                  </p>
                </div>
              ...
              {!isAuthenticated && (
                <div className="mt-4 bg-blue-50 p-4 rounded-md border border-blue-100">
                  <p className="text-sm text-blue-700 mb-2">
                    <strong>Checking out as a guest?</strong> You can continue
                    without an account, but signing in offers benefits:
                  </p>
