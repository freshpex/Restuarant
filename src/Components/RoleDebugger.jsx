...
import { selectCurrentUser, selectToken, selectIsAdmin } from '../redux/selectors';
  ...
  return (
    <div className="fixed bottom-4 right-4 bg-gray-800 text-white p-4 rounded-lg shadow-lg max-w-sm z-50 opacity-80 hover:opacity-100 transition-opacity">
      ...
      <div className="text-sm">
        <div><span className="font-bold">UID:</span> {user.uid}</div>
        <div><span className="font-bold">Email:</span> {user.email}</div>
        <div><span className="font-bold">Role in state:</span> {user.role || 'Not set'}</div>
        <div><span className="font-bold">Is Admin:</span> {isAdmin ? 'Yes' : 'No'}</div>
        
        {serverCheck && (
          <>
            <hr className="my-2 border-gray-600" />
            <div><span className="font-bold">Server Check:</span></div>
            <div><span className="font-bold">- Role:</span> {serverCheck.role}</div>
            <div><span className="font-bold">- Is Admin:</span> {serverCheck.isAdmin ? 'Yes' : 'No'}</div>
            <div><span className="font-bold">- User in DB:</span> {serverCheck.userInDb ? 'Yes' : 'No'}</div>
          </>
        )}
