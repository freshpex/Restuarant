...
const PaymentModal = ({
  ...
  const totalPrice =
    orderDetails.totalPrice || (unitPrice * quantity).toFixed(2) + deliveryFee;
    ...
    >
      <div className="relative bg-white rounded-lg p-6 w-11/12 max-w-md mx-auto shadow-2xl">
            ...
            <div className="mt-2 space-y-1">
              ...
              <div className="flex justify-between">
                <p className="text-sm text-gray-600">
                  Delivery to{" "}
                  {capitalizeWords(
                    orderDetails.deliveryLocation || "Not specified",
                  )}
                </p>
                <p className="text-sm text-gray-600">
          ...
          <button
            onClick={() => {
              if (isPaymentLoading) {
                if (
                  window.confirm(
                    "Payment is in progress. Cancelling now may result in a failed transaction. Are you sure you want to cancel?",
                  )
                ) {
                  onClose();
                }
        ...
        {orderDetails.paymentStatus && (
          <p
            className={`text-sm ${orderDetails.paymentStatus === "paid" ? "text-green-600" : "text-yellow-600"} mt-2 text-center font-medium`}
          >
        ...
        <p className="text-xs text-gray-500 text-center mt-4">
          You can chat with our chef for customization or pay online to confirm
